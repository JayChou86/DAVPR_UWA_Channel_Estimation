%% =================================================================
%  Channel_review_824.m — 深度感知传感器节点通信性能评估
%  深度: 824.8m (节点C, 下温跃层 ★网络瓶颈节点)
%  Scope: 水下声学传感器网络 (UASN) 固定节点
%
%  修改内容 (v2.0): 自动多径提取 + 自动参数选择 + RLS vs LMS 对比
% =================================================================
cc
addpath('utils');

%% 1. 加载信道数据并预处理
depth_label = '824.8m (Node C, Lower Thermocline)';
load('时变冲激响应824_0.1_3.mat')
...
title(sprintf('BER Performance: %s', depth_label), 'FontSize', 13);
legend(sprintf('RLS-DFE (fb=%d, \\lambda=%.4f)', eq_fb_taps, rls_forget), ...
       sprintf('LMS-DFE (fb=%d)', params.eq_fb_taps_lms), 'Location', 'SouthWest');
axis([min(SNR_vec) max(SNR_vec) 1e-5 1]);

save_folder = '..\五月份实测数据\误码率数据0到30';
if ~exist(save_folder, 'dir'), mkdir(save_folder); end
filename_base = sprintf('824_fb%d_lambda%.4f', eq_fb_taps, rls_forget);
