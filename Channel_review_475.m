...
eq_fb_taps = 90;         % 反馈抽头数 核心
rls_forget = 0.995;      % RLS 遗忘因子 (接近1稳定，小则跟踪快)
SNR_vec = 0:5:30; % dB
...
% eq_ff_taps = 60;         % 前馈抽头数 (根据多径长度调整)
% eq_fb_taps = 90;         % 反馈抽头数 核心
% rls_forget = 0.999;      % RLS 遗忘因子 (接近1稳定，小则跟踪快)
...
h_matrix_cut = zeros(74,400);
%%
h_matrix_cut(:,[10,361]) = abs(result_matrix(:,[100,451]));
% h_matrix_cut(:,380) = abs(result_matrix(:,380));
...
filename = sprintf('475_%g_%g.png', eq_fb_taps,rls_forget); % 文件名
filename2 = sprintf('475_%g_%g.mat', eq_fb_taps,rls_forget); % 文件名
fig_path = fullfile(save_folder, sprintf('475_%g_%g.fig', eq_fb_taps,rls_forget));
saveas(gcf, fig_path, 'fig');
png_path = fullfile(save_folder, sprintf('475_%g_%g.png', eq_fb_taps,rls_forget));
saveas(gcf, png_path, 'png');
mat_path = fullfile(save_folder, sprintf('475_%g_%g.mat', eq_fb_taps,rls_forget));
save(mat_path, 'BER_vec');
disp('图像已保存');
% save(filename2,"BER_vec")


% figure
% plot(tx_chunk(1:end-29-5-4+6))
% hold on
% plot(rx_chunk(35+4-6:end))
% xlim([1,50])
