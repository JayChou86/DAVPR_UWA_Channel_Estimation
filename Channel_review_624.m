...
rls_forget = 0.998;      % RLS 遗忘因子 (接近1稳定，小则跟踪快)
SNR_vec = 0:5:30; % dB
...
h_matrix_cut = zeros(74,2400);
%%
h_matrix_cut(:,[100,271,610,2341]) = abs(result_matrix(:,[100,271,610,2341]));
%%
...
filename = sprintf('624_%g_%g.png', eq_fb_taps,rls_forget); % 文件名
fig_path = fullfile(save_folder, sprintf('624_%g_%g.fig', eq_fb_taps,rls_forget));
saveas(gcf, fig_path, 'fig');
png_path = fullfile(save_folder, sprintf('624_%g_%g', eq_fb_taps,rls_forget));
saveas(gcf, png_path, 'png');
mat_path = fullfile(save_folder, sprintf('624_%g_%g.mat', eq_fb_taps,rls_forget));
