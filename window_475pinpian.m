...
% Y/X
%% 同步
max_window_duration = 5; % 最大窗长度, 单位: 秒
...
fs = gcd_abc;
dt = 1/fs;
% sig_bd = sig_bd(1:180*fs);
% sig_ori = sig_ori(1:180*fs);
...
% plotfft(sig_rev, 1)
% plotfft(sig_bd, 1)
% plotfft(sig_rev_sync(934609:940716), fs)
% plotfft(sig_bd_sync(934609:940716), fs)



% 截出m序列
sig_bd_m = sig_bd_sync(951667:1117040);
sig_rev_m = sig_rev_sync(951667-371:1117040-371);

% cor(sig_rev_m, sig_bd_m, 1)


% f_bd = 592.339;      % 标定信号中的单频频率（Hz）
% f_received = 592.346;    % 接收信号中的单频频率（Hz）
% 
% alpha_bd = f_received / f_calib;
% [P_tx1, Q_tx1] = rat(alpha_bd);
% sig_rev = resample(sig_rev, P_tx1, Q_tx1); % 重采样法补偿换能器频偏
...
for window_duration = 1:max_window_duration
    ...
    % --- 其他可选的归一化方式 ---
    % 按Frobenius范数归一化
    % h_matrix = h_matrix / norm(h_matrix, 'fro');
    % 使每个时间点(每一行)的能量为1
    % h_matrix = h_matrix ./ vecnorm(h_matrix, 2, 2);
    % h_matrix(isnan(h_matrix)) = 0; % 处理可能出现的NaN (如果某行为全零)


    % 绘制热力图 (Time-Varying Impulse Response)
    figure;
    ...
    colormap('jet'); % 设置颜色图
    xlim([-20, 500]) % 根据需要设置时延显示范围

    % save_folder = '..\五月份实测数据\图像\475'; % 修改为您的目标文件夹
